...
/**
 * 임시 미션 추천 로직. 실제 AI 추천 서비스가 붙기 전까지, 하드코딩된 미션 풀에서 랜덤으로 하나를 골라 반환한다. 추후 AI 연동 시 이 클래스(또는 이 클래스가 구현하는
 * 인터페이스)만 교체하면 된다.
 */
@Component
public class MissionTemplateProvider {

    private static final List<MissionTemplate> TEMPLATES =
            List.of(
                    new MissionTemplate(
                            "목적 없이 10분 걷기",
                            "10분 동안 목적지 없이 걷다가 멈춘 곳을 찍어보아요.",
                            MissionCategory.OBSERVATION,
                            true,
