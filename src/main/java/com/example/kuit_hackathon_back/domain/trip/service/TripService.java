...
public class TripService {
    ...
    public CurrentTripResponse getCurrentTrip(Long userId) {
        return tripRepository.findByUser_UserIdAndStatus(userId, TripStatus.ACTIVE)
                .map(trip -> CurrentTripResponse.of(trip, buildMissionSummary(trip.getTripId()),
    ...
    public CreateTripResponse createTrip(Long userId, CreateTripRequest request) {
        if (request.startDate().isAfter(request.endDate())) {
            throw new BusinessException(ErrorCode.VALIDATION_ERROR, "여행 기간이 올바르지 않습니다.",
    ...
    public TripReviewResponse getTripReview(Long userId, Long tripId) {
        Trip trip = getOwnedTripOrThrow(userId, tripId);
        long success = missionRepository.countByTrip_TripIdAndMissionStatus(tripId, MissionStatus.SUCCESS);
        long failed = missionRepository.countByTrip_TripIdAndMissionStatus(tripId, MissionStatus.FAILURE);
        ...
        return new TripReviewResponse(trip.getTripId(), trip.getTripName(), trip.getRegion(), trip.getStartDate(),
                trip.getEndDate(), trip.getCompanionType(), trip.getMood(), trip.getStatus(),
                success, failed, total, totalCollections);
    }

    private MissionSummary buildMissionSummary(Long tripId) {
        long total = missionRepository.countByTrip_TripId(tripId);
        long active = missionRepository.countByTrip_TripIdAndMissionStatus(tripId, MissionStatus.ACTIVE);
        long success = missionRepository.countByTrip_TripIdAndMissionStatus(tripId, MissionStatus.SUCCESS);
        long failed = missionRepository.countByTrip_TripIdAndMissionStatus(tripId, MissionStatus.FAILURE);
    ...
    private CollectionSummary buildCollectionSummary(Long tripId) {
        ...
        long success = collectionRepository.countByTrip_TripIdAndStatus(tripId, CollectionStatus.SUCCESS);
        long failed = collectionRepository.countByTrip_TripIdAndStatus(tripId, CollectionStatus.FAILURE);
